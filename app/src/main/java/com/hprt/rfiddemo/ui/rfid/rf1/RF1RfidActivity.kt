...
import kotlinx.android.synthetic.main.activity_rfid_rf1.*
import org.simple.eventbus.Subscriber
import org.simple.eventbus.ThreadMode
import java.util.*

class RF1RfidActivity : BaseActivity() {
    var Awl: AndroidWakeLock? = null
    var mProgressDialogUtil:ProgressDialogUtil?=null

    var rF1InventoryFragment = RF1InventoryFragment.newInstance()
    ...
    private val serviceConnection by lazy {
        object : ServiceConnection {
            ...
            override fun onServiceConnected(name: ComponentName, service: IBinder) {
                ...
                rF1InventoryFragment.arguments = bundle
                ...
                var settingFragment = RF1SettingFragment.newInstance()
                ...
                mFragments.add(rF1InventoryFragment)
    ...
    override fun getContentView(): Int = R.layout.activity_rfid_rf1

    override fun initView() {
        topbar.setTitle("RF1-RFID")
    ...
    override fun onKeyDown(keyCode: Int, event: KeyEvent): Boolean {
        if (keyCode == KeyEvent.KEYCODE_F10) {
            if (mFragments != null && mFragments.size >= 1 && RFHelper.isConnect) {
                ...
                (fragment as RF1InventoryFragment).onKeyDown()
            }
            return true
        }
    ...
    override fun onKeyUp(keyCode: Int, event: KeyEvent?): Boolean {
        if (keyCode == KeyEvent.KEYCODE_F10) {
            if (mFragments != null && mFragments.size >= 1 && RFHelper.isConnect) {
                val fragment = mFragments[0]
                (fragment as RF1InventoryFragment).onKeyUp()
            }
            return true
        }
    ...
    override fun onBackPressed() {
        val fragment = mFragments[0]  as RF1InventoryFragment
    ...
    public fun handleLost(string: String) {
        rF1InventoryFragment?.stopInventory()
        rF1InventoryFragment?.stopTimer()
        binder?.disconnect()
    }
